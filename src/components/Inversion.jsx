...
import { Card, SectionTitle, MonoAmount, Row, Input, Select, btnPrimary, btnGhost, btnSmall, iconBtn, pillBtn } from "./ui.jsx";
...
export function Inversion({ t, data, update }) {
  ...
  function remove(id) { update({ investments: data.investments.filter((i) => i.id !== id) }); if (editingId === id) resetForm(); }
  ...
  function removeFromWatchlist(id) { update({ watchlist: data.watchlist.filter((w) => w.id !== id) }); }
    ...
    <div>
      <Card t={t}>
        ...
        <div style={{ borderTop: `1px solid ${t.border}`, marginTop: 8, paddingTop: 8, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <MonoAmount t={t} value={totalGain} color={totalGain >= 0 ? t.green : t.red} />
        </div>
      </Card>
      ...
      <Card t={t}>
        <SectionTitle t={t}>Mercado en tiempo real — watchlist</SectionTitle>
