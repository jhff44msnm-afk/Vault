...
export function ConfirmProvider({ children, t }) {
  const [state, setState] = useState({ open: false, message: "", confirmLabel: "Eliminar", onConfirm: null, onCancel: null });

  const confirm = useCallback((message, confirmLabel = "Eliminar") => {
      ...
      {state.open && (
        <div style={{ position: "fixed", inset: 0, zIndex: 9998, display: "flex", alignItems: "center", justifyContent: "center", animation: "vault-overlayIn 0.2s ease" }}>
          ...
          <div style={{
            ...
            <div style={{ fontSize: 15, fontWeight: 700, color: t.text, marginBottom: 8 }}>Confirmar</div>
